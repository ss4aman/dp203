...
class LinkEntity {
    ...
    async _negotiateClaim({ abortSignal, setTokenRenewal, timeoutInMs, }) {
        ...
        if (!this._context.cbsSession.isOpen()) {
            await coreAmqp.defaultCancellableLock.acquire(this._context.cbsSession.cbsLock, () => this._context.cbsSession.init({
                abortSignal,
        ...
        await coreAmqp.defaultCancellableLock.acquire(this._context.negotiateClaimLock, () => this._context.cbsSession.negotiateClaim(this.audience, tokenObject.token, tokenType, {
            abortSignal,
            timeoutInMs: timeoutInMs - (Date.now() - startTime),
        }), {
            abortSignal,
    ...
    _ensureTokenRenewal() {
        ...
        this._tokenRenewalTimer = setTimeout(() => this._negotiateClaim({
            setTokenRenewal: true,
            timeoutInMs: getRetryAttemptTimeoutInMs(),
        }).catch((err) => logger.verbose("[%s] %s '%s' with address %s, an error occurred while renewing the token: %O", this._context.connectionId, this._type, this.name, this.address, err)), this._tokenTimeoutInMs);
    ...
    get _type() {
        var _a;
        return (_a = this.constructor.name) !== null && _a !== void 0 ? _a : "LinkEntity";
    }
}
...
class EventHubReceiver extends LinkEntity {
    ...
    abort() {
        ...
        (_a = this._onError) === null || _a === void 0 ? void 0 : _a.call(this, new abortController.AbortError(coreAmqp.StandardAbortMessage));
        return this.close();
    }
    /**
    ...
    _createReceiverOptions(options) {
        ...
        if (this.options.trackLastEnqueuedEventProperties) {
            receiverOptions.desired_capabilities = coreAmqp.Constants.enableReceiverRuntimeMetricName;
        }
    ...
    async receiveBatch(maxMessageCount, maxWaitTimeInSeconds = 60, abortSignal) {
        ...
        const tryOpenLink = () => coreUtil.createAbortablePromise((resolve, reject) => this.initialize({
            abortSignal,
...
function waitForEvents(maxEventCount, maxWaitTimeInMs, readIntervalWaitTimeInMs, queue, options) {
    ...
    const { signal: abortSignal } = new abortController.AbortController([
        aborter.signal,
